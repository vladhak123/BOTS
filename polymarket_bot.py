...
def call_deepseek(prompt: str, max_tokens: int = 600) -> str:
    # Using Claude claude-sonnet-4-20250514 API
    r = requests.post(
        "https://api.anthropic.com/v1/messages",
        headers={
            "x-api-key": os.environ.get("CLAUDE_KEY", ""),
...
async def _do_analyse(message):
    ...
    slug = matched.get("slug") or matched.get("conditionId") or matched.get("id", "")
...
async def _do_autostart(message, ctx):
    ...
    ctx.job_queue.run_repeating(
        _auto_job, interval=1800, first=30,
...
if __name__ == "__main__":
    main()    history = data.get("balance_history", [])
    history.append({"ts": datetime.now(timezone.utc).isoformat(), "balance": round(data["balance"], 2)})
    data["balance_history"] = history[-200:]
    with open(MEMORY_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
